...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/03/31 03:21:11 by teevee            #+#    #+#             */
/*   Updated: 2021/03/31 04:24:38 by teevee           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			print_status(t_phil *ph, char *s)
{
	...
	while (--time >= 0)
		mtime += 1000;
	if (pthread_mutex_lock(&ph->common->op_mutex))
		return (1);
	...
	while (*s)
		write(1, s++, 1);
	write(1, "\n", 1);
	if (pthread_mutex_unlock(&ph->common->op_mutex))
		return (1);
	return (0);
}
